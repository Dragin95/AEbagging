    ...
    def feed_data(self, fqueue, epoch):
        ...
        for ekey in train:
            ...
            for skey in event:
                station = event[skey] 
                data = station[:]
                pt, st, snr = station.attrs["p_pick"], station.attrs["s_pick"], station.attrs["SNR"]
                fqueue.put([data, [int(pt), int(st)], snr])
    def process(self, fqueue, dqueue):
        count = 0
        llen = self.length//self.stride
        while True:
            data, pidx, snr = fqueue.get()
            snrs = snr
            pdic = {"P":pidx[0],"S":pidx[1]}
            # 做增强的，随机一个位置
            bidx = np.random.choice(pidx) - np.random.randint(self.padlen, self.length-self.padlen)
            ...
            for pkey in pdic:
                ...
                if idx > 0 and idx < self.length:
                    phase_time[pid] = idx    
            dqueue.put([rdata, [phase_time[0], phase_time[1]], snrs])
            count += 1

    def batch_data(self, batch_size=50):
        x1, x2, x3 = [], [], []
        for _ in range(batch_size):
            data, label1, snrss = self.dqueue.get() 
            x1.append(data) 
            x2.append(label1) 
            x3.append(snrss) 
            # x3.append(snrr), snrr, x3
        x1 = np.stack(x1, axis=0) 
        return x1, x2, x3
